...
#else
    const char* pszModule = "Bitcoin Invest BTCI";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bitcoin Invest BTCI
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bitcoin Invest BTCI
    // Mac: ~/Library/Application Support/Bitcoin Invest BTCI
    // Unix: ~/.Bitcoin Invest BTCI
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BTCICoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "btci.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "btcid.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Bitcoin Invest BTCI will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("Bitcoin Invest BTCI"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
