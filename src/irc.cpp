...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("Bitcoin Invest BTCI-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #Bitcoin Invest BTCITEST2\r");
            Send(hSocket, "WHO #Bitcoin Invest BTCITEST2\r");
        } else {
            // randomly join #Bitcoin Invest BTCI00-#Bitcoin Invest BTCI05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Bitcoin Invest BTCI%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Bitcoin Invest BTCI%02d\r", channel_number).c_str());
        }
