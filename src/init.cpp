...
void Shutdown(void* parg)
{
    ...
    RenameThread("Bitcoin Invest BTCI-shutoff");
...
//        CTxDB().Close();
        ...
        Sleep(50);
        printf("Bitcoin Invest BTCI exited\n\n");
        fExit = true;
#ifndef QT_GUI
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("Bitcoin Invest BTCI version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  BTCId [options]                     " + "\n" +
                  "  BTCId [options] <command> [params]  " + _("Send command to -server or BTCId") + "\n" +
                  "  BTCId [options] help                " + _("List commands") + "\n" +
                  "  BTCId [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "Bitcoin Invest BTCI:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("Bitcoin Invest BTCI"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("Bitcoin Invest BTCI"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: btcid.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: btcid.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 30010 or testnet: 40010)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 30005 or testnet: 40005)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    //if (fTestNet) {
        SoftSetBoolArg("-irc", false); // NO IRC
    //}
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  Bitcoin Invest BTCI is probably already running."), strDataDir.c_str()));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("Bitcoin Invest BTCI version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "Bitcoin Invest BTCI server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("Bitcoin Invest BTCI"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Bitcoin Invest BTCI") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart Bitcoin Invest BTCI to complete") << "\n";
