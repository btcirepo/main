...
unsigned int nStakeMinAge = 24 * nOneHour; // one day minimum for 2% POS reward
unsigned int nStakeMaxAge = 24 * nOneHour; //  one day maximum for 2% POS reward
unsigned int nStakeTargetSpacing = nOneHour*0.125; // 96 cycles of POS staking
int64_t nTargetSpacing = nStakeTargetSpacing;  // Same as the above
unsigned int nModifierInterval = nStakeTargetSpacing; // time to elapse before new modifier is computed
...
const string strMessageMagic = "Bitcoin Invest BTCI Signed Message:\n";
...
{
    int64_t nRewardCoinYear = 2 * COIN; //2% dziennie

    int64_t nSubsidy = nCoinAge * nRewardCoinYear / 100;
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("Bitcoin Invest BTCI-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Bitcoin Invest BTCI
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // Bitcoin Invest BTCI: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Bitcoin Invest BTCI", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "BTCI GENESIS";
        CTransaction txNew;
        txNew.nTime = 1535373264;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	...
	block.print();
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xc10e6128cf5a167bade14835ada90b0b3e5f0f8f1f0d3e5959f99f16b0127346"));
...
unsigned char pchMessageStart[4] = { 0xb1, 0xb2, 0xb3, 0xb4 };
